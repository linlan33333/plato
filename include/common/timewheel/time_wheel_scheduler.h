...
public:
  ...
  uint32_t CreateTimerAt(int64_t when_ms, const TaskFunc& task);
  uint32_t CreateTimerAfter(int64_t delay_ms, const TaskFunc& task);
  uint32_t CreateTimerEvery(int64_t interval_ms, const TaskFunc& task);

  // 取消一个定时任务，这个任务id得自己保存然后传进来
